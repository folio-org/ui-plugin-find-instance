...
const cqlParser = (intl) => {
  ...
  CQLParser.prototype = {
    parse(query) {
      if (!query) throw new Error(intl.formatMessage({ id: 'ui-plugin-find-instance.parser.emptyQuery' }));
      this.qs = query;
      this.ql = this.qs.length;
      this.qi = 0;
      this._move();
      this.tree = this._parseQuery(this.scf, this.scr, []);
      if (this.look !== '') throw new Error(intl.formatMessage({ id: 'ui-plugin-find-instance.parser.eof' }));
    },
    toString() {
    ...
    _parseModifiers() {
      ...
      while (this.look === '/') {
        ...
        if (this.look !== 's' && this.look !== 'q') {
          throw new Error(intl.formatMessage({ id: 'ui-plugin-find-instance.parser.invalidModifier' }));
        }
        ...
        if (this.look.length > 0
          ...
          this._move();
          if (this.look !== 's' && this.look !== 'q') {
            throw new Error(intl.formatMessage({ id: 'ui-plugin-find-instance.parser.invalidRelation' }));
          }
    ...
    _parseSearchClause(field, relation, modifiers) {
      if (this.look === '(') {
        ...
        if (this.look === ')') {
          ...
          // throw new Error(intl.formatMessage({ id: 'ui-plugin-find-instance.parser.missingCloseParenthesis' }));
        }
        return b;
        ...
        this._move();
        if (this.look !== 's' && this.look !== 'q') {
          throw new Error(intl.formatMessage({ id: 'ui-plugin-find-instance.parser.strOrQuotedExpression' }));
        }
        ...
        throw new Error(intl.formatMessage({ id: 'ui-plugin-find-instance.parser.invalidSearchClause' }));
      }
    },
    _move() {
      // eat whitespace
