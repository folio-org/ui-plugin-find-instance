...
jest.mock('./FindInstanceContainer', () => {
  const RealFindInstanceContainer = jest.requireActual('./FindInstanceContainer').default;
...
describe('InstanceSearch', () => {
  ...
  describe('when isMultiSelect is applied and a user selects instances and clicks save', () => {
    it('should call useInstancesQuery with the selected instances', async () => {
      ...
      const { getByText, queryByTestId, rerender } = renderInstanceSearch(props);

      await act(async () => userEvent.click(getByText('plugin-trigger')));

      rerender(getInstanceSearch(props));

      await act(async () => userEvent.click(document.getElementById('checkbox-33')));
